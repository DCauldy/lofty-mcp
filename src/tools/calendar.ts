...
export function registerCalendarTools(server: McpServer) {
  server.tool(
    ...
    "Query calendar events (tasks and appointments) in Lofty CRM (V2 Calendar API).",
    {
      ...
      startTimeMs: z.number().optional().describe("Start time as Unix timestamp in milliseconds"),
      endTimeMs: z.number().optional().describe("End time as Unix timestamp in milliseconds"),
      timeZoneCode: z.string().optional().describe("IANA timezone (e.g. America/Los_Angeles)"),
      includeFinished: z.boolean().optional().describe("Include finished events (default false)"),
      page: z.number().optional().describe("Page number (starts from 0)"),
